...
def train_agent(SYMBOL, agent_id):
    ...
    while not trained:
        ...
        while True:
            #env.render()
            action = choose_action(agent, state, eps)  # select an action
        ...
        if (len(avgs)%10) == 0:
            print("AGENT", agent_id, "act", la, "assess", behaviour_assess, "rate", behaviour_assess[1] / behaviour_assess[-1])
            ...
            if avg > best_score:
                torch.save(agent.qnetwork_local.state_dict(), SYMBOL+ '_' + agent_id +'_daily_trader.pt')
                best_score = avg
